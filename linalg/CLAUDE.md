...
App ôn tập Đại số tuyến tính (khung Strang / MIT 18.06), menu theo việc (Tổng quan · Học ·
Luyện tập · Thi thử — D16).
Kế thừa nguyên khung của `project_kmap`; khác biệt duy nhất về kiến trúc là
có thêm tầng `src/geometry/`.

## Nguyên tắc
- Mỗi file một trách nhiệm, mục tiêu dưới ~400 dòng, trần 450 (D40 TOEIC) (`npm run check` ở gốc repo).
...
- `logic/` và `geometry/` **không được** biết ngôn ngữ: ném `AppError(key, params)`
  (`shared/logic/app-error.js`) hoặc trả về `labelKey/reasonKey/textKey` + params.
...
`main.js` rồi gắn bằng `mountLesson(host, md, {chapter, banks, figures})`. Hai bản phải cùng
...
Chạy `npm run dev` ở gốc repo → http://localhost:5180/linalg/.
