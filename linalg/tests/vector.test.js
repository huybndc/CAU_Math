...
describe('spanKind2', () => {
  it('điểm, đường, mặt phẳng', () => {
    expect(V.spanKind2([0, 0], [0, 0])).toBe('point');
    expect(V.spanKind2([1, 2], [2, 4])).toBe('line');
    expect(V.spanKind2([1, 0], [0, 0])).toBe('line');
    expect(V.spanKind2([1, 0], [0, 1])).toBe('plane');
  });
});
