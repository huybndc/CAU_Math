...
/* ---------------- cổng 2 hoặc 3 ngõ vào (bài luyện: cột F theo tên cổng) ---------------- */
export const GATE_NAMES = ['AND', 'OR', 'NAND', 'NOR', 'XOR', 'XNOR'];
const GATE_FN = {
  AND: b => (b.every(x => x === 1) ? 1 : 0), OR: b => (b.some(x => x === 1) ? 1 : 0),
  NAND: nandN, NOR: norN, XOR: xorN, XNOR: b => xorN(b) ^ 1,
};

/** Cột F (chuỗi bit) của cổng `name` có `n` ngõ vào, dòng đầu là 00…0 (bit cao nhất). */
export function gateBits(name, n) {
  let out = '';
  for (let m = 0; m < 1 << n; m++) out += GATE_FN[name]([...m.toString(2).padStart(n, '0')].map(Number));
  return out;
}

/** Ký hiệu đại số của cổng n ngõ vào: 'x · y · z', '(x + y + z)′'… */
export function gateSymbol(name, n) {
  const v = ['x', 'y', 'z'].slice(0, n);
  const base = { AND: v.join(' · '), NAND: v.join(' · '), OR: v.join(' + '), NOR: v.join(' + '), XOR: v.join(' ⊕ '), XNOR: v.join(' ⊕ ') }[name];
  return ['NAND', 'NOR', 'XNOR'].includes(name) ? `(${base})′` : base;
}
