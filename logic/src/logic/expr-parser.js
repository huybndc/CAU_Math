...
import { fail as appFail } from '@shared/logic/app-error.js';
...
/**
 * Đếm số literal của một biểu thức bất kỳ (SOP, POS, có ngoặc): mỗi lần một
 * biến xuất hiện là một literal. Dùng để so "độ tối giản" của đáp án người học
 * với lời giải tối ưu, kể cả khi đáp án viết dạng POS.
 */
export function countLiterals(text, n) {
  const names = varNames(n);
  return [...String(text)].filter(ch => names.includes(ch.toLowerCase())).length;
}

/** Số biến theo tên biến Mano có trong biểu thức (x,y → 2; có z → 3; có w → 4; có v → 5). */
export function varsCount(...exprs) {
  const s = exprs.join(' ').toLowerCase();
  if (/v/.test(s)) return 5;
  if (/w/.test(s)) return 4;
  if (/z/.test(s)) return 3;
  return 2;
}
